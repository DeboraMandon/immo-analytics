"""
taux_scraper.py — Scraping des taux de crédit immobilier
...
# Données de repli — mise à jour manuelle mensuelle recommandée
# Source : CAFPI baromètre public mai 2026
FALLBACK_TAUX = [
    {"duree_ans": 10, "taux_moyen": 2.99, "taux_min": 2.70, "taux_max": 3.20,
     "taux_usure": 5.19, "oat_10ans": 3.75, "source": "CAFPI_manual"},
    {"duree_ans": 15, "taux_moyen": 3.20, "taux_min": 2.90, "taux_max": 3.45,
     "taux_usure": 5.19, "oat_10ans": 3.75, "source": "CAFPI_manual"},
    {"duree_ans": 20, "taux_moyen": 3.33, "taux_min": 3.05, "taux_max": 3.55,
     "taux_usure": 5.19, "oat_10ans": 3.75, "source": "CAFPI_manual"},
    {"duree_ans": 25, "taux_moyen": 3.43, "taux_min": 3.20, "taux_max": 3.65,
     "taux_usure": 5.19, "oat_10ans": 3.75, "source": "CAFPI_manual"},
]


async def scrape_cafpi() -> list[dict]:
    """Parse le baromètre CAFPI (page publique)."""
    try:
...
async def insert_taux(conn, rows: list[dict]) -> int:
    """Insère les taux en base avec upsert."""
    inserted = 0
    today = date.today().replace(day=1)
...
async def main():
    ...
    try:
        ...
        if not rows:
            ...
            rows = [{**r, "date_obs": date.today().replace(day=1)} for r in FALLBACK_TAUX]

        n = await insert_taux(conn, rows)

        # Log pipeline
        await conn.execute("""
            INSERT INTO pipeline_log (dag_id, source, type, status, records, duration_s)
            VALUES ($1, $2, $3, $4, $5, $6)
        """, "taux_scraper", rows[0]["source"], "taux", "success",
             n, round(time.monotonic() - t0, 2))

        logger.info(f"✅ {n} taux insérés en {time.monotonic() - t0:.1f}s")
        ...
        await conn.execute("""
            ...
            VALUES ($1, $2, $3, $4, $5)
        """, "taux_scraper", "CAFPI", "taux", "error", str(e))
        raise
